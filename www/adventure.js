  ...
  return `
    <div class="adv-step">
      ...
      <div class="adv-reminders-section">
        ...
        <div id="advReminderList">
          ${d._aiReminders ? renderAIRemindersHTML(d._aiReminders) : `
            <button class="adv-ai-remind-btn" onclick="loadAIReminders()">
...
function renderAIRemindersHTML(reminders) {
  return reminders.map((r, i) => `
    <label class="adv-reminder-opt">
      <input type="checkbox" id="advRemindSmart${i}" ${r.checked ? 'checked' : ''} data-reminder-key="${escapeHtmlUI(r.key)}" data-reminder-label="${escapeHtmlUI(r.label)}">
...
async function loadAIReminders() {
  ...
  if (!container) return;
  container.innerHTML = '<div style="text-align:center;padding:12px;color:var(--text-tertiary);font-size:.85rem">✨ Generating smart reminders...</div>';
  ...
  try {
    ...
    const context = [
      ...
      'Generate exactly 4 specific preparation reminder items. Each should have an emoji, a short title, and timing.'
    ].filter(Boolean).join('. ');
    ...
    if (resp.ok) {
      ...
      if (reminders.length > 0) {
        ...
        container.innerHTML = renderAIRemindersHTML(reminders);
        return;
      }
    }
...
function renderFallbackReminders(container) {
  ...
  if (/paddle|surf|kayak|swim|lake|beach|water|ocean/.test(all)) {
    ...
    r.push({key:'w2',label:'🌊 Check water & weather — 1 day before',checked:true});
  } else if (/ski|snowboard|snow|winter/.test(all)) {
  ...
  container.innerHTML = renderAIRemindersHTML(d._aiReminders);
}
